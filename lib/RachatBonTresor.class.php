...
left join devise d1 on t1.id_devise = d1.id_devise)' ;
		...
		class ScriptLgn1RachatBonTresorTradPlatf extends ScriptEditRachatBonTresorTradPlatf
		{
			...
			protected function ChargeFormPrinc()
			{
				...
				$this->FltDateRachat = $this->FormPrinc->InsereFltEditHttpPost('date_emission', 'date_emission') ;
				$this->FltDateRachat->Libelle = "Date valeur" ;
				$this->CompDateRachat = $this->FltDateRachat->DeclareComposant("PvCalendarDateInput") ;
				...
				$this->FournDonneesPrinc->RequeteSelection = "(select t1.*, DATEDIFF(date_echeance, date_emission) + 1 duree_emission from rachat_bon_tresor t1)" ;
		...
		class CritrValidRachatBonTresorTradPlatf extends PvCritereBase
		{
			public function EstRespecte()
			{
				...
				$valDateRachat = $this->ScriptParent->FltDateRachat->Lie() ;

<?php
	...
	{
		define('ARCH_OP_CHANGE_TRAD_PLATF', 1) ;
		
		class ScriptConsultArchOpChangeTradPlatf extends PvScriptWebSimple
		{
	...
	<div id="vente-devise">
		<iframe src="?appelleScript=consultArchVenteDevise" style="width:100%" height="'.$this->HauteurCadre.'" frameborder="0"></iframe>
	</div>
</div>
<script language="javascript">
	jQuery(function() {
		...
		class ScriptConsultArchElemOpChangeTradPlatf extends PvScriptWebSimple
		{
	...
	where date(t1.date_change) = date(t2.date_change)
		...
		class ScriptConsultArchAchatDeviseTradPlatf extends ScriptConsultArchElemOpChangeTradPlatf
		{
			public $IdTypeChange = 1 ;
		}
		class ScriptConsultArchVenteDeviseTradPlatf extends ScriptConsultArchElemOpChangeTradPlatf
		{
			public $IdTypeChange = 2 ;
		}
		
		class ScriptDetailArchOpChangeTradPlatf extends PvScriptWebSimple
		{
			public $FormPrinc ;
			public $FltIdEnCours ;
			...
			protected function ChargeFormPrinc()
			{
				...
				$this->FormPrinc->DessinateurFiltresEdition = new DessinFltEditArchOpChangeTradPlatf() ;
...
where date(t1.date_change) = date(t2.date_change)
		...
		class DessinFltEditArchOpChangeTradPlatf extends PvDessinateurRenduHtmlFiltresDonnees
		{
